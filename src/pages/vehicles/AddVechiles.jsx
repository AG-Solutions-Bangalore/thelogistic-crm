...
const AddVechiles = () => {
  ...
  const fetchCompany = async () => {
    try {
      ...
      console.error("Error fetching vechile compnay data", error);
    } finally {
      setLoading(false);
    }
  };
  const fetchDriver = async () => {
    try {
      ...
      const response = await axios.get(
        `${BASE_URL}/api/web-fetch-assigning-newdriver/${vehicles.vehicle_branch}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      console.error("Error fetching vechile driver data", error);
    } finally {
      setLoading(false);
    }
  };
  const fetchBranch = async () => {
    try {
      ...
      console.error("Error fetching vechiles braanch data", error);
    } finally {
      setLoading(false);
    }
  };
